...
Core tests take longer than any other Mangonote tests, due to the high amount of computational work involved in validating core components.
...
To run only Mangonote's core tests (after building):
...
To run only Mangonote's crypto tests (after building):
...
```bash
mangonoted --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `mangonote-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Mangonote's hash tests (after building):
...
To run only Mangonote's performance tests (after building):
...
To run only Mangonote's unit tests (after building):
