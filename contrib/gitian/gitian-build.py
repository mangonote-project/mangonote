...
gsigs = 'https://github.com/mangonote-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('mangonote'):
        # Remove the potentially stale mangonote dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'mangonote', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'mangonote'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/mangonote/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'mangonote='+args.commit, '--url', 'mangonote='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/mangonote-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/mangonote/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/mangonote/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/mangonote-project/mangonote', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/mangonote', exist_ok=True)
    os.chdir('builder/inputs/mangonote')
    if args.pull:
