# The Current/Future Status of ZMQ in Mangonote

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Mangonote
...
matching supported by both Mangonote and ZMQ. The `format`, `context` and `event`
...
The Mangonote daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Mangonote daemon also ensures that
...
The Mangonote daemon will send a `txpool_add` pub exactly once for each
